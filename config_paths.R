...
# Project root
# Avoid `%||%` so this works on base R < 4.4 without dplyr/rlang attached.
.this_file <- tryCatch(sys.frame(1)$ofile, error = function(e) NULL)
if (is.null(.this_file) || !nzchar(.this_file)) .this_file <- "."
PROJECT_ROOT <- normalizePath(dirname(.this_file), winslash = "/", mustWork = FALSE)
rm(.this_file)
